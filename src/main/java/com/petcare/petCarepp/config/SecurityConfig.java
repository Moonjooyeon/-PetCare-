...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .***REMOVED***2Login(***REMOVED***2 -> ***REMOVED***2
                        .userInfoEndpoint(userInfo -> userInfo
                                .userService(customOAuth2UserService)
                        )
                        .successHandler((request, response, authentication) -> {
                            CustomOAuth2User ***REMOVED***User = (CustomOAuth2User) authentication.getPrincipal();

                            // JWT 토큰 생성
                            String token = jwtTokenProvider.generateToken(***REMOVED***User.getEmail());

                            // 프론트엔드 리다이렉션 주소로 이동
                            String redirectUrl = "http://localhost:5173/welcome?token=" + token +
                                    "&userName=" + URLEncoder.encode(***REMOVED***User.getName(), StandardCharsets.UTF_8);
